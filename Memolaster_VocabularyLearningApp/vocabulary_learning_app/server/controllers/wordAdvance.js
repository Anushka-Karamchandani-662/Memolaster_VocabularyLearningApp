const advanceWordModel=require('../models/advanceWords')

let currentPage = 1; // Initialize current page

module.exports.getWordsss = async (req, res) => {
    const pageSize = 1; // Number of words to retrieve per page

    try {
        const totalCount = await advanceWordModel.countDocuments({}); // Get total count of words in the database
        ...
        const data = await advanceWordModel.find({}).skip(skip).limit(pageSize); // Retrieve words for the current page
