...
module.exports.recommendWords = async (req, res) => {
    const pageSize = 1; // Number of words to retrieve per page

    try {
        const totalCount = await RecommendModel.countDocuments({}); // Get total count of words in the database
        ...
        const data = await RecommendModel.find({}).skip(skip).limit(pageSize); // Retrieve words for the current page
